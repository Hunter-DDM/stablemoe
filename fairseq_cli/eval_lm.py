...
def eval_lm(
    ...
    for step, sample in enumerate(batch_iterator):
        # ! handle the synchronous problem, skip the last batch
        if step == len(batch_iterator) - 1:
            continue
        if "net_input" not in sample:
            continue
        # # ! handle pad_to_bsz, skip the last unaligned batch
        # if len(sample["id"]) != sample["net_input"]["src_tokens"].shape[0]:
            # continue
        ...
        for i, hypos_i in enumerate(hypos):
            # # ! handle pad_to_bsz, skip extra examples
            # if i >= len(sample["id"]):
            #     continue
            hypo = hypos_i[0]
